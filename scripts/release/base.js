...
const filters = ["subjekt", "subjektify", "@subjektifylabs/*"];
...
const updateDependencies = (pkg) => {
  ...
  const filteredDependencies = Object.keys(peerDependencies).filter((dep) =>
    filters.some((filter) => dep.includes(filter)),
  );

  console.log(
    `${chalk.yellow("[DEBUG]")} Filtered dependencies:`,
...
const commitChanges = (pkg) => {
  ...
  execCommand(`git add .`);
  execCommand(
    `git commit -m "chore(${pkg}): update dependencies and bump version"`,
  );
...
const _release = async (pkg, type) => {
  console.log(
    `${chalk.yellow("[DEBUG]")} Releasing ${type} version for ${pkg}...`,
  );
  ...
  console.log(
    `${chalk.green("[SUCCESS]")} Released ${type} version for ${pkg}`,
  );
  await sleep(30);
};
