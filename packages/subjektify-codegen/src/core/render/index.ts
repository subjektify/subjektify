...
export const render = async (config: CodeGenConfig, model: SubjektModel) => {
  ...
  const resolvedOutputDirectory = outputDirectory
    ...
    : path.resolve(process.cwd(), target, version || "latest");
  Log.info(
    `Rendering template for ${target} in ${language} to ${resolvedOutputDirectory}...`,
  );
  ...
  try {
    ...
    for (const file of files) {
      ...
      const outputFileName = file.replace(
        ".eta",
        language === "solidity" ? ".sol" : ".ts",
      );
      const outputPath = path.join(resolvedOutputDirectory, outputFileName);
      const outputContent = eta.render(templateName, {
        model,
        target,
        language,
        outputDirectory,
    ...
    Log.error(
      `Failed to render templates for ${target} in ${language}: ${error}`,
    );
    throw error;
  }
};
